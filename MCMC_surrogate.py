...
import model_surrogate_GOSATSIF as models
...
def posterior(parms):
    ...
    #Uniform priors
    prior = 1.0
    parms[2:6]=model.pdef[2:6]
    parms[7:] = model.pdef[7:]
    ...
    if (prior > 0.0):
        ...
        for v in range(0,len(myoutput)):
            if (abs(myobs[v]) < 1e5):
                ...
                post = post + li
                #print(v, myoutput[v], myobs[v], post)
    else:
        post = -9999999
    return(post)
...
def MCMC(parms, nevals, type='uniform', nburn=1000, burnsteps=10, doplots=True):
    ...
    for i in range(0,nevals):
        ...
        if (i > 0 and (i % nburn) == 0 and i < burnsteps*nburn):
            ...
            mycov_chain = numpy.cov(chain_burn[0:nparms,int(accepted_tot/4):accepted_tot])
            thisscalefac = 1.0
            if (acc_ratio <= 0.2):
        ...
        if (i == burnsteps*nburn and doplots):
        ...
        if (len(model.output) == model.nobs):
   ...
   #parameter correlation plots (threshold correlations)
    ...
    for p1 in range(0,nparms-1):
      for p2 in range(p1+1,nparms):
        if (abs(parmcorr[p1,p2]) > corr_thresh and doplots):
          fig = plt.figure()
...
pfts_torun = [1,2,3,4,5,6,7,8,10,11,12,13,14,15]
#pfts_torun = [4]
for p in pfts_torun:
  model = models.MyModel(pft=p)
  model.get_obs(pft=p)
  model.run(model.pdef)

  parms = MCMC(model.pdef, 100000, burnsteps=10, nburn=4000, doplots=True)
  os.system('rm -rf plots_pft'+str(p))
  os.system('mv plots plots_pft'+str(p))
  os.system('mkdir -p /compyfs/www/ricc364/SIF_optimization')
  os.system('rm -rf /compyfs/www/ricc364/SIF_optimization/plots_pft'+str(p))
  os.system('cp -r plots_pft'+str(p)+' '+'/compyfs/www/ricc364/SIF_optimization')
  os.system('mv chain.txt chain_pft'+str(p)+'.txt')
