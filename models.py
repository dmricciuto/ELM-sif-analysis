...
class MyModel(object):
    ...
    def __init__(self, name):
        self.name = name
        if (self.name == 'Simple_NEE'):
            self.nparms = 4
            self.parm_names = ['Rbase', 'Q10', 'GPPmax', 'PARhalf']  #Parameter names
            self.pmin = numpy.array([0., 1., 1., 100.])
            self.pmax = numpy.array([10., 3.5, 25., 1000.])
            self.pdef = numpy.array([2., 2., 20., 500.])
        elif (self.name == 'DALEC'):
            self.nparms = 22
            ...
            self.nparms = 3
            self.parm_names = ['Rbase', 'Q10', 'LUE']  #Parameter names
            self.pmin = numpy.array([0., 1., 1.])
            self.pmax = numpy.array([10., 3.5, 25])
            self.pdef = numpy.array([2., 2., 20])
    ...
    def run(self, parms):
        ...
        #Run the model 
        if (self.name == 'Simple_NEE'):
            self.tair = (self.tmax+self.tmin)/2.0
            self.output = parms[0]*parms[1]**((self.tair-10)/10.) - \
                          parms[2]*self.rad/(self.rad+parms[3])
        elif (self.name == 'DALEC'):
                #initial pool sizes
    ...
    def load_forcings(self, site):
         myinput = open('./'+site+'_drivers.csv','r')
         self.tmin = numpy.array([])
         self.tmax  =  numpy.array([])
         self.rad = numpy.array([])
         self.cair = numpy.array([])
         self.doy = numpy.array([])
         self.dayl = numpy.array([])
         self.x = numpy.array([])
         self.nobs = 0
         for s in myinput:
             if (self.nobs < 365):
                 self.tmin = numpy.append(self.tmin,float(s.split(',')[1])-273.15)
                 self.tmax = numpy.append(self.tmax,float(s.split(',')[2])-273.15)
                 self.rad  = numpy.append(self.rad, float(s.split(',')[3]))
                 self.cair = numpy.append(self.cair,float(s.split(',')[4]))
                 self.doy  = numpy.append(self.doy,float(self.nobs % 365)+1)
                 self.x    = numpy.append(self.x,(self.nobs)/365.0)
         ...
         for d in range(0,self.nobs):
             ...
             if (mult >= 1.):
                 self.dayl = numpy.append(self.dayl,24.0)
             elif (mult <= -1.):
                 self.dayl = numpy.append(self.dayl,0.)
             else:
                 self.dayl = numpy.append(self.dayl, 24.*numpy.arccos(-mult)/numpy.pi)
